...
class BracketGuideHighlightingPassTest : BasePlatformTestCase() {
    override fun setUp() {
        super.setUp()
    ...
    fun testDocumentChangesReleaseStaleAnalysisButKeepTokenPresentation() {
        ...
        assertTrue(EditorGuideSessions.hasAcceptedAnalysis(editor, acceptedStamp))
    ...
    fun testActiveToTokenOnlyRebuildsCompactAnalysisBeforeRestoringActivePresentation() {
        ...
        assertEquals(1, collections)
        assertTrue(EditorGuideSessions.hasAcceptedAnalysis(editor, fullStamp))
        assertNotNull(activeGuide())
        ...
        assertFalse(EditorGuideSessions.hasAcceptedAnalysis(editor, fullStamp))
        assertFalse(EditorGuideSessions.hasAcceptedAnalysis(editor, tokenOnlyStamp))
    ...
    fun testLateFullPassCannotRestoreASnapshotAfterAllFeaturesAreDisabled() {
        ...
        assertFalse(EditorGuideSessions.hasAcceptedAnalysis(editor, fullStamp))
        assertTrue(EditorGuideSessions.hasAcceptedAnalysis(editor, disabledStamp))
        assertTrue(ownedHighlighters().isEmpty())
    ...
    private fun applyPass(pass: BracketGuideHighlightingPass) {
        inReadAction {
            pass.doCollectInformation(EmptyProgressIndicator())
        }
        pass.doApplyInformationToEditor()
    ...
    private fun activeGuideState(): BracketGuideDrawing? =
    ...
    private fun applyOptions(options: BracketGuidePreferences) {
        BracketGuideSettings.getInstance().replace(options)
        session().updateOptions(
            options,
            resolveImmediately = true,
