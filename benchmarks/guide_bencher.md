...
1. Create or select a public project in your Free Bencher account. Copy its
   project slug and create a project-scoped API key.
2. Open **Settings → Secrets and variables → Actions** in this GitHub repository.
   Set repository variable `BENCHER_PROJECT` to the slug and repository secret
   `BENCHER_API_KEY` to the API key. See the
   [Bencher GitHub Actions guide](https://bencher.dev/docs/how-to/github-actions/).
3. Run **Actions → Benchmark Jobs → Run workflow** on `main` after the workflow
   change is merged. This creates the initial baseline. Subsequent matching
   pushes to `main` and eligible PR updates run automatically.
...
Setting `BENCHER_PROJECT` switches same-repository PRs, main pushes, and manual
runs to Bencher; a missing API key then fails the run. Without that variable,
and for fork or Dependabot PRs, the existing GitHub runner matrix validates
execution and coverage without uploading results.
...
Configured workflow runs share one concurrency group and queue up to 100 pending
runs with `queue: max`; new runs do not replace pending measurements. Workflow
runs do not automatically cancel an active run. See
...
not cancel an already submitted remote job. Remove `BENCHER_PROJECT` to return
future runs to the standard GitHub validation matrix.
