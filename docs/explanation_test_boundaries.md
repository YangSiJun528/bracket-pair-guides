...
이번 변경에서는 snapshot query, 실제 document 분석, caret 결과, editor markup,
설정 적용처럼 제품이 제공하는 행위를 테스트의 기본 관찰 지점으로 삼았다.
...
반대로 `GuideTreeShape`의 16 MiB 제한이나 `DaemonRestartContract`의 버전별
method 선택은 실제 제품의 메모리·호환성 정책이다. 이들은 테스트를 위해 만든
훅이 아니라 production 경로가 직접 사용하는 협력 객체로 분리했다.
...
| tree 지원 여부·payload getter | 실제 메모리 정책 | `GuideTreeShape`로 분리하고 production index도 직접 사용 |
| `CaretBracketSearch` budget·clock 주입 | 구현 제어 seam | 제거; 일반 입력 성공과 실제 512-transition 소진 결과를 검증 |
...
4 ms elapsed ceiling은 실제 단조 시계를 사용하는 best-effort UI guard다. wall
clock 합격선을 단위 테스트로 두면 scheduler와 테스트 장비를 검증하게 되므로,
가짜 clock seam과 함께 해당 테스트를 삭제했다. 결정적인 512-transition 제한과
일반 입력의 정상 결과는 계속 검증한다. 현재 JMH는 pairing과 sort 경계만
측정하므로 4 ms 정책을 정량 회귀 계약으로 만들려면 caret 전용 benchmark를
별도로 추가해야 한다. test-only clock을 다시 노출하는 것으로 대신하지 않는다.
...
| `routeDocumentChangeForTest` | 실제 전달 정책 | `DocumentChangeRoute`로 분리하고 production event도 사용 |
...
| `documentChanged` 기본 인자 | test 편의 default | 제거; 전달 정책이 즉시 처리 여부를 명시 |
...
1. `BracketAnalysis`, `BracketSnapshot`, `CaretContext`의 use-case 결과
2. `GuideTreeShape`, `DocumentChangeRoute`, `DaemonRestartContract`처럼 production이
   직접 사용하는 정책 객체
...
- engine: 106개 테스트 성공, 실패·오류 0개.
- plugin: 102개 테스트 성공, 실패·오류 0개.
- `:engine:check`, `:plugin:check`와 engine ABI·root-package 검사 성공.
- `:plugin:verifyPluginProjectConfiguration`, `:plugin:verifyPluginStructure` 성공.
- benchmark의 실제 engine 경로 컴파일과 JMH smoke 38개 조합 실행 성공.
- production의 `TestOnly`, `VisibleForTesting`, `*ForTest`, test-only 명명 검색 결과 0개.
- 프로젝트 소유 `-er`, `-or` 타입 선언 검색 결과 0개.
- `git diff --check` 성공.
