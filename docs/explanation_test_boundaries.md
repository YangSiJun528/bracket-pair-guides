# 과거 구현 보고서: 프로덕션 경계와 테스트 설계

> **문서 상태:** 테스트 전용 프로덕션 노출을 제거한 작업의 당시 감사
> 결과와 검증 기록을 보존하는 과거 보고서다. 신규 기여자를 위한 현재
> 책임 경계는 [아키텍처](explanation_architecture.md), 현재 테스트 실행
> 방법은 [기여 가이드](../CONTRIBUTING.md), 현재 성능 수치와 한도는
> [성능 및 용량 레퍼런스](reference_performance_limits.md)를 기준으로
> 한다. 이 보고서의 파일명, 테스트 수, 수치는 현재 상태를 보장하지
> 않는다.

이 보고서는 Bracket Pair Guides에서 테스트가 프로덕션 API와 객체 책임에
어떤 영향을 주어야 하는지 판단한 당시의 근거를 기록한다. 테스트 작성법이나
현재 구조를 안내하는 문서가 아니라, 기존 테스트 전용 노출을 어떻게 판정하고
재설계했는지 보존한다.

## 결론

보고서 작성 시점의 프로젝트에는 프로덕션 코드의 `@TestOnly`, `@VisibleForTesting`,
