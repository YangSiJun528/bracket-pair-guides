...
- Load compatibility: `src/main/resources/META-INF/plugin.xml`, the explicitly
  pinned `since-build="241"`, JetBrains' recommended Plugin Verifier matrix,
  and the explicit IntelliJ IDEA 2026.2 endpoint in `build.gradle.kts`.
- Runtime fixture: `build.gradle.kts` and the repository language regression
  tests.
